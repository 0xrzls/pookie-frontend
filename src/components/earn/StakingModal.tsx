...
export default function StakingModal({ isOpen, onDismiss, stakingInfo, userLiquidityUnstaked }: StakingModalProps) {
  ...
  async function onAttemptToApprove() {
    if (!pairContract || !signer || !deadline) throw new Error('missing dependencies')
      ...
      {!attempting && !hash && (
        <ContentWrapper gap="lg">
          ...
          <CurrencyInputPanel
            ...
            pair={dummyPair}
            label={''}
            renderBalance={(amount) => <span>Available to deposit: {formatCurrencyAmount(amount, 4)}</span>}
      ...
      {attempting && !hash && (
        <LoadingView onDismiss={wrappedOnDismiss}>
          <AutoColumn gap="12px" justify={'center'}>
            <TYPE.largeHeader>
              <span>Depositing Liquidity</span>
            </TYPE.largeHeader>
            <TYPE.body fontSize={20}>
              <span>{parsedAmount?.toSignificant(4)} UNI-V2</span>
            </TYPE.body>
          </AutoColumn>
        </LoadingView>
      )}
      {attempting && hash && (
        <SubmittedView onDismiss={wrappedOnDismiss} hash={hash}>
          <AutoColumn gap="12px" justify={'center'}>
            ...
            <TYPE.body fontSize={20}>
              <span>Deposited {parsedAmount?.toSignificant(4)} UNI-V2</span>
            </TYPE.body>
          </AutoColumn>
        </SubmittedView>
      )}
