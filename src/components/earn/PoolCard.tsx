      ...
      <TopSection>
        ...
        <StyledInternalLink to={`/uni/${currencyId(currency0)}/${currencyId(currency1)}`} style={{ width: '100%' }}>
          <ButtonPrimary padding="8px" $borderRadius="8px">
            {isStaking ? <span>Manage</span> : <span>Deposit</span>}
          </ButtonPrimary>
        </StyledInternalLink>
      </TopSection>
        ...
        <RowBetween>
          ...
          <TYPE.white>
            {valueOfTotalStakedAmountInUSDC ? (
              <span>${valueOfTotalStakedAmountInUSDC.toFixed(0, { groupSeparator: ',' })}</span>
            ) : (
              <span>${valueOfTotalStakedAmountInWETH?.toSignificant(4, { groupSeparator: ',' }) ?? '-'} ETH</span>
            )}
          </TYPE.white>
        </RowBetween>
        <RowBetween>
