...
const RootWrapper = styled.div<{ chainId: SupportedChainId; darkMode: boolean; logoUrl: string }>`
  ${({ chainId, darkMode }) =>
    chainId === SupportedChainId.KAKAROT
      ? darkMode
  ...
  return (
    <RootWrapper chainId={chainId} darkMode={darkMode} logoUrl={info.logoUrl}>
      ...
      <ContentWrapper>
        ...
        <Header>
          <span>PookieSwap on {NETWORK_LABELS[chainId]}</span>
        </Header>
        <Body>
          <span>
            This is an alpha release of PookieSwap on the {NETWORK_LABELS[chainId]} network. You must bridge L1 assets
            to the network to swap them.
          </span>
        </Body>
      </ContentWrapper>
      <LinkOutToBridge href={info.bridge} target="_blank" rel="noopener noreferrer">
        <span>Deposit to {NETWORK_LABELS[chainId]}</span>
        <LinkOutCircle />
      </LinkOutToBridge>
    </RootWrapper>
  )
