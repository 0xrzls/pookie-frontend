...
import { Currency, CurrencyAmount, Percent, Token } from 'dependencies/sdk-core'
...
import { ReactComponent as DropDown } from '../../assets/svg/arrow-down.svg'
...
const InputPanel = styled.div<{ hideInput?: boolean }>`
  ...
  background-color: ${({ theme, hideInput }) => (hideInput ? 'transparent' : '#F5F5F5')};
  z-index: 1;
...
const CurrencySelect = styled(ButtonGray)<{ selected: boolean; hideInput?: boolean }>`
  ...
  font-size: 24px;
  font-weight: 500;
  /* background-color: ${({ selected, theme }) => (selected ? '#3F3CFF' : '#3F3CFF')}; */
  ...
  /* :focus,
  :hover {
    background-color: ${({ selected, theme }) => (selected ? darken(0.05, '#3F3CFF') : darken(0.05, '#3F3CFF'))};
  } */
`
...
const StyledDropDown = styled(DropDown)<{ selected: boolean }>`
  ...
  path {
    stroke: ${({ selected, theme }) => (selected ? '#262626' : '#262626')};
    stroke-width: 1.5px;
  }
`
                ...
                {pair ? (
                  ...
                  <StyledTokenName className="token-symbol-container" active={Boolean(currency && currency.symbol)}>
                    {(currency && currency.symbol && currency.symbol.length > 20
                      ...
                      : currency?.symbol) || <span>Choose Token</span>}
                  </StyledTokenName>
                )}
              </RowFixed>
